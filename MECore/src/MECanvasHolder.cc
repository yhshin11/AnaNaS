...
: fCanvas(0), 
  fWelcomePave(0), fWelcomeState(true), fWelcomeTitle(0), fWelcomeL0(0), 
  fTexTL(0), fTexTR(0), fTexBL(0), fTexBR(0), 
  _h(0), _scale(1), _refw( 1092 )
{
}
...
void 
MECanvasHolder::SetCanvas( TCanvas* canvas )
  //, const char* textTL, const char* textBL, const char* textTR, const char* textBR ) 
{
  assert( canvas!=0 );
  ...
  //  setComments( textTL, textBL, textTR, textBR );

}

void 
MECanvasHolder::ShowWelcome( bool image ) 
{
  gPad->Clear();
  if( image )
    {
...
void 
MECanvasHolder::setSessionStyle()
{
  ...
  gStyle->SetPadLeftMargin(_scale*0.155);
  ...
  fCanvas->SetFillColor(kGray+2);
  fCanvas->SetGridx(0);
  fCanvas->SetGridx(1);

  fPad->SetLeftMargin(0.12);
  fPad->SetRightMargin(0.12);
...
void 
MECanvasHolder::setHistoStyle( TH1* h )
{
  if( h==0 ) return;
...
void 
MECanvasHolder::setPxAndPy( int px, int py ) 
{ 
  _px=px; _py=py; 
  _x=0; _y=0;
  if( _h!=0 )
    {
      ...
      // Yuk !!!
      //
      int istart1 = objectInfo.Index("(x="); int iend1 = istart1+3;
      int istart2 = objectInfo.Index(", y="); int iend2 = istart2+4;
...
void
MECanvasHolder::setComments( const char* textTL, const char* textBL, const char* textTR, const char* textBR )
{  
  ...
  delete fTexTL; fTexTL = new TLatex(fXTexTL,fYTexTL,fStringTL.Data());
  delete fTexTR; fTexTR = new TLatex(fXTexTR,fYTexTR,fStringTR.Data());
  delete fTexBL; fTexBL = new TLatex(fXTexBL,fYTexBL,fStringBL.Data());
  delete fTexBR; fTexBR = new TLatex(fXTexBR,fYTexBR,fStringBR.Data());
  // from DrawLabels()
  //Draws the 4 labels on the Canvas
  fTexTL->SetTextFont(fFontTex);
  fTexTL->SetTextSize(fSizeTex);
  fTexTL->SetTextColor(fColorTex);
  fTexTL->SetLineWidth(fWidthTex);
  fTexTL->SetTextAlign(11);
  fTexBL->SetTextFont(fFontTex);
  fTexBL->SetTextSize(fSizeTex);
  fTexBL->SetTextColor(fColorTex);
  fTexBL->SetLineWidth(fWidthTex);
  fTexBL->SetTextAlign(13);
  fTexBR->SetTextFont(fFontTex);
  fTexBR->SetTextSize(fSizeTex);
  fTexBR->SetTextColor(fColorTex);
  fTexBR->SetLineWidth(fWidthTex);
  fTexBR->SetTextAlign(33);
  fTexTR->SetTextFont(fFontTex);
  fTexTR->SetTextColor(fColorTex);
  fTexTR->SetTextSize(fSizeTex);
  fTexTR->SetLineWidth(fWidthTex);
  fTexTR->SetTextAlign(31);
  fCanvas->cd();
  fTexTL->Draw();
  fTexBL->Draw();
  fTexBR->Draw();
