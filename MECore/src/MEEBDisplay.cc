...
map< int, TPolyLine* > MEEBDisplay::_xyXtals;
  ...
  for( int iSM=1; iSM<=36; iSM++ ) 
      ...
      for( int iX=0; iX<=16; iX++ )
	{
	  for( int iY=0; iY<=3; iY++ )
	    {
	      drawTT( iSM, iX, iY, 0., etamin, etamax, phimin, phimax );
	    }
	}
    }
}
...
{
  TPolyLine* pline = getTTPolyLine( iSM, iX, iY, shift, etamin, etamax, phimin, phimax );
  if ( pline!=0 )
    {
      registerTObject( pline );
  ...
  if( shift==0 )
    {
      //      if( iSM==1 || iSM==19 )  
      drawTT( iSM, iX, iY, 2., etamin, etamax, phimin, phimax );
      drawTT( iSM, iX, iY, -2., etamin, etamax, phimin, phimax );
    }
}
...
{
  TPolyLine* pline = getXtalPolyLine( ieta, iphi, shift, etamin, etamax, phimin, phimax );
  if( pline==0 ) return;
  registerTObject( pline );
  ...
  if( shift==0 )
    {
      ...
      if( iSM==1 || iSM==19 )  drawXtal( ieta, iphi, color, 2., etamin, etamax, phimin, phimax );
    }
}

void
MEEBDisplay::drawRz( int lc, int fc )
{
  ...
  for( int ii=1; ii<=85; ii++ )
    {
      _rzXtals[ii]->SetLineColor( lc );
      _rzXtals[ii]->SetLineWidth( 1 );
      _rzXtals[ii]->SetFillColor( fc );
      _rzXtals[ii]->Draw("f");
      _rzXtals[ii]->Draw();
...
// GHM feb12
void
MEEBDisplay::drawXy( int lc, int fc )
{
  // simplified version!
  //  double R_ = 129.124;
  double R_ = 130.000;
  //  double L_ = 152.100-R_;
  double L_ = 23.;
  double Dphi_ = 0.95*acos(-1.)/180.;
  double phi0_ = Dphi_/2.;

  // phi=1
  double ebxx[10][5]; double ebyy[10][5];
  for( int ii=0; ii<10; ii++ )
    {
      double phi_ = phi0_+Dphi_/2.;
      double LL_ = R_/cos(phi_)+L_/cos(Dphi_/2.);
      ebxx[ii][0]= R_; ebyy[ii][0]= R_*tan(phi_);
      ebxx[ii][1]=  cos(Dphi_)*ebxx[ii][0] + sin(Dphi_)*ebyy[ii][0];
      ebyy[ii][1]= -sin(Dphi_)*ebxx[ii][0] + cos(Dphi_)*ebyy[ii][0];
      ebxx[ii][3]= LL_*cos(phi_); ebyy[ii][3]= LL_*sin(phi_);
      ebxx[ii][2]=  cos(Dphi_)*ebxx[ii][3] + sin(Dphi_)*ebyy[ii][3];
      ebyy[ii][2]= -sin(Dphi_)*ebxx[ii][3] + cos(Dphi_)*ebyy[ii][3];
      ebxx[ii][4]=   ebxx[ii][0]; ebyy[ii][4]=   ebyy[ii][0];
      phi0_ += Dphi_;
    }

  for( int ii=0; ii<10; ii++ )
    {
      double xx_[5]; double yy_[5];
      double phi_  = 0;
      double dphi_ = 2*acos(-1.)/18.;
      for( int jj=0; jj<18; jj++ )
	{ 
	  for( int kk=0; kk<5; kk++ )
	    {
	      xx_[kk] = ebxx[ii][kk]*cos(phi_)-ebyy[ii][kk]*sin(phi_);
	      yy_[kk] = ebxx[ii][kk]*sin(phi_)+ebyy[ii][kk]*cos(phi_);
	    }
	  _xyXtals[ii+20*jj+ 1] = new TPolyLine( 5, xx_,yy_);
	  for( int kk=0; kk<5; kk++ )
	    {
	      xx_[kk] = ebxx[ii][kk]*cos(phi_)+ebyy[ii][kk]*sin(phi_);
	      yy_[kk] = ebxx[ii][kk]*sin(phi_)-ebyy[ii][kk]*cos(phi_);
	    }
	  _xyXtals[ii+20*jj+11] = new TPolyLine( 5, xx_,yy_);
	  phi_ += dphi_;
	} 
    }
  for( int ii=1; ii<=360; ii++ )
    {
      _xyXtals[ii]->SetLineColor( lc );
      _xyXtals[ii]->SetLineWidth( 1 );
      _xyXtals[ii]->SetFillColor( fc );
      _xyXtals[ii]->Draw("f");
      _xyXtals[ii]->Draw();
      ...
      //      for( int jj=1; jj<5; jj++ )
	{
	  pline_ = (TPolyLine*) _xyXtals[ii]->Clone();
	  //	  for( int kk=0; kk<5; kk++ ) 
	  //	    {
	  //	      pline_->GetX()[kk] *= -1;
	  //	    }
...
MEEBGeom::EtaPhiPoint 
MEEBDisplay::center( MEEBGeom::EBGlobalCoord ieta, MEEBGeom::EBGlobalCoord iphi  )
{  
  int iSM = MEEBGeom::sm( ieta, iphi );
  ...
  for( int ii=0; ii<4; ii++ )
    {
      ...
      if( eta[ii]<etamin || eta[ii]>etamax ) ok_= false;
      if( phi[ii]<phimin || phi[ii]>phimax ) ok_= false;
    }
  eta[4] = eta[0];
  phi[4] = phi[0];
  if( !ok_ ) return 0;
...
{  
  ...
  pair<float,float> p2_(phioverpi_1,phioverpi_0);
  _phiLimits.insert( pair< int, pair<float,float> >( 10*iSM+(19-iy), p2_ ) );
  pair<float,float> p20_(phioverpi_0,phioverpi_1);
  _phiLimits.insert( pair< int, pair<float,float> >( 10*(iSM+18)+iy, p20_ ) );
}
...
MEEBDisplay::setSM_2_and_20()
{
  ...
  //  cout << "entering setSM_2_and_20 " << endl;
...
//   setPhiLimits(	2,	19,	21,	0.0617,	0.0564);
  ...
  setPhiLimits(	2,	0,	21,	0.1661,	0.1607);
  setPhiLimits(	2,	1,	22,	0.1606,	0.1552);
  setPhiLimits(	2,	2,	23,	0.1551,	0.1497);
  setPhiLimits(	2,	3,	24,	0.1496,	0.1442);
  setPhiLimits(	2,	4,	25,	0.1441,	0.1387);
  setPhiLimits(	2,	5,	26,	0.1386,	0.1333);
  setPhiLimits(	2,	6,	27,	0.1331,	0.1277);
  setPhiLimits(	2,	7,	28,	0.1276,	0.1223);
  setPhiLimits(	2,	8,	29,	0.1221,	0.1168);
  setPhiLimits(	2,	9,	30,	0.1166,	0.1113);
  setPhiLimits(	2,	10,	31,	0.1111,	0.1058);
  setPhiLimits(	2,	11,	32,	0.1057,	0.1003);
  setPhiLimits(	2,	12,	33,	0.1001,	0.0948);
  setPhiLimits(	2,	13,	34,	0.0947,	0.0893);
  setPhiLimits(	2,	14,	35,	0.0892,	0.0838);
  setPhiLimits(	2,	15,	36,	0.0837,	0.0783);
  setPhiLimits(	2,	16,	37,	0.0782,	0.0728);
  setPhiLimits(	2,	17,	38,	0.0727,	0.0673);
  setPhiLimits(	2,	18,	39,	0.0672,	0.0618);
  setPhiLimits(	2,	19,	40,	0.0617,	0.0564);
