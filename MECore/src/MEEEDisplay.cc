...
TPolyLine* 
MEEEDisplay::getSCPolyLine( int iX, int iY, int iz, float shift, float etamin, float etamax, float phimin, float phimax )
{
...
MEEEDisplay::center( int ix, int iy, int iz )
{
  int iX = (ix-1)/5+1;
  int iY = (iy-1)/5+1;
  int iSC = MEEEGeom::sc( iX, iY );
  ...
  if( iSC<0 )           
    {
      cout << "WARNING -- ISC=0 " << endl;
      return MEEEGeom::EtaPhiPoint(1000,0);
    }

  int crystal_in_sc = MEEEGeom::crystal_in_sc( ix, iy );
  if( crystal_in_sc<0 ) 
    {
      cout << "WARNING -- XTAL IN SC=0" << endl;
      return MEEEGeom::EtaPhiPoint(1000,0);
    }
  ...
  eta_ /= 4;
  phi_ /= 4;
  return MEEEGeom::EtaPhiPoint( eta_, phi_ );
}

TPolyLine* 
MEEEDisplay::getXtalPolyLine( int ix, int iy, int iz, float shift, float etamin, float etamax, float phimin, float phimax )
{
  int iX = (ix-1)/5+1;
  int iY = (iy-1)/5+1;
  int iSC = MEEEGeom::sc( iX, iY );
...
void 
MEEEDisplay::drawEE( float etamin, float etamax, float phimin, float phimax )
{
  //  refresh();
  //  int iz=1;
  for( int iz=-1; iz<=1; iz+=2 )
      ...
      for( int iX=1; iX<=20; iX++ )
	{
	  for( int iY=1; iY<=20; iY++ )
	    {
	      drawSC( iX, iY, iz, 0., etamin, etamax, phimin, phimax );
	    }
	}
    }
}
...
void 
MEEEDisplay::drawSC( int iX, int iY, int iz, float shift,  float etamin, float etamax, float phimin, float phimax  )
{
  TPolyLine* pline = getSCPolyLine( iX, iY, iz, shift, etamin, etamax, phimin, phimax );
  if( pline!=0 )
    {
  ...
  if( shift==0 )
    {
      //      if( iY==10 && iX>=11 ) 
      drawSC( iX, iY, iz, -2., etamin, etamax, phimin, phimax );
      //      if( iY==11 && iX>=11 ) 
      drawSC( iX, iY, iz,  2., etamin, etamax, phimin, phimax );
    }
}
...
void 
MEEEDisplay::drawXtal( int ix, int iy, int iz, int color, float shift,  float etamin, float etamax, float phimin, float phimax )
{
  TPolyLine* pline = getXtalPolyLine( ix, iy, iz, shift, etamin, etamax, phimin, phimax );
  if( pline==0 ) return;
  registerTObject( pline );
  ...
  if( shift==0 )
    {
      int iX = (ix-1)/5+1;
      int iY = (iy-1)/5+1;
      if( iY==10 && iX>=11 ) drawXtal( ix, iy, iz, color, -2., etamin, etamax, phimin, phimax );
      if( iY==11 && iX>=11 ) drawXtal( ix, iy, iz, color,  2., etamin, etamax, phimin, phimax );
    }
}
...
void
MEEEDisplay::drawRz( int lc, int fc )
{
  setRzXtals();
