...
class ZZAnalysis : public SampleAnalysis
{
public:

  ZZAnalysis( Sample& sample, const std::string & collectionFileName  );
  virtual ~ZZAnalysis();
...
private:
  ...
  bool LeptonIso();
  bool LepIso(Candidate* dau_);
  bool LeptonID();
  bool LepID(Candidate* dau_);
...
//   void GetElNNMETValue();
  ...
  void fillProjectedMET(Candidate* ZCand);
  bool fillVertices();

  float ComputeResolution(float E);

  ClassDef( ZZAnalysis, 0 )  
};
