#include "Analysis/src/ZZAnalysis.hh"
...
int ZZAnalysis::imode = EventServer::kZmm;

ClassImp( ZZAnalysis )
  
  ZZAnalysis::ZZAnalysis( Sample& sample, const std::string & collectionFileName ):
  SampleAnalysis( "ZZAnalysis" , sample, collectionFileName ),
  /* metNN_(0),*/ Primary(0),
  outTree_("METCommVariables","Variables for MET Splots")
{
  ...
  SampleAnalysis::defineTemplate( "dxy", 100,0,1);
  SampleAnalysis::defineTemplate( "nChi2", 1000,0,10);
...
ZZAnalysis::HLTFiring() {
  ...
  else if(SampleName.substr(0,1)=="M") {

    // cout<< _e.isFired("HLT_Mu9")<<"   "<<_e.isFired("HLT_Mu15_v1")<<"   "<<_e.isFired("HLT_Mu15_v2")<<endl;
...
bool
ZZAnalysis::EventPreselection(Candidate* ZCand) {
  ...
  for( int idau=0; idau<2; idau++ )
	...
	categ_[idau]="E";
      ...
      if(Zmode=="Zee" && Et_> 20 && 
	 ( fabs(eta_)<1.4442 || (fabs(eta_)>1.56 && fabs(eta_)<2.5) ) )
	Presel[idau]=true;
      if(Zmode=="Zmm" && Et_> 20 && ( fabs(eta_)<2.1 ) )
	Presel[idau]=true;
    }
  ...
  for( int idau=0; idau<2; idau++ )
    {
      ...
      acc[idau] = LepIso(dau_);

      /*  if(Zmode=="Zee") {
	.007  0.03  0.
	float trkIso = info_->getFloat("dr03TrkIso")/dau_->pt();
	float ecalIso = info_->getFloat("dr04EcalIso")/dau_->pt();
	float hcalIso = (info_->getFloat("dr04HcalD1Iso") + info_->getFloat("dr04HcalD2Iso") )/dau_->pt() ;
	...
	if(etaSep > fabs(info_->getFloat("caloEta") ) ) { //barrel
	  ...
	  if(trkIso < TrkIso[1] && ecalIso < EcalIso[1] &&
		...
		float sumIso = info_->getFloat("isoR03strk")/dau_->pt();
...
bool
ZZAnalysis::LepIso(Candidate* dau_) {
  ...
  //Electron Working Point, passing to 80% 
  /* TrkIso[0]= 0.15; TrkIso[1]= 0.05;
  EcalIso[0]= 2.0; EcalIso[1]= 0.06;
  HcalIso[0]= 0.12; HcalIso[1]= 0.05;*/
  ...
  if(abs(dau_->pdgCode()) == 11 )//electron
    {
      etaSep=1.479;
  ...
  if(abs(dau_->pdgCode()) == 13 ) //muon
    {
      //No Iso for easier control after
      //FIXME
      //MM
      acc =true;
      /*   etaSep=1.3;
...
ZZAnalysis::LeptonID() {
  ...
  for( int idau=0; idau<2; idau++ ) {
	...
	if( hoe < HOE[1] && sigieie < sigiEiE[1] && 
      ...
      float nvHits =  info_->getInt("nValTrackerHits"); //info_->getInt("nValMuHits") +
      
      if(info_->getBool("muidGlobalMuonPromptTight") == true &&
	 (nvHits>10) )
        acc[idau] = true;
  ...
  if(acc[0] && acc[1] && Zmode=="Zee")
    accept=true;
  if(acc[0] && acc[1] &&  Zmode=="Zmm" && (muID[0] || muID[1]) )
    accept=true;
  // cout<<Zmode<<"   "<<acc[0]<<"   "<<acc[1]<<"   "<<muID[0]<<"   "<<muID[1]<<endl;
  return accept;
}

bool
ZZAnalysis::LepID(Candidate* dau_) {
  ...
  if(abs(dau_->pdgCode())==13) //muon
    {
      ...
      fill("n10","muIDGlobal", info_->getBool("muidGlobalMuonPromptTight") );
      if(info_->getBool("muidGlobalMuonPromptTight") == true && (nvHits>=10) )
        acc = true;
      
      if( !MuonSpecID(dau_)) acc =false;
    }
...
bool
ZZAnalysis::MuonSpecID(Candidate* mu) {

  CandInfo* info = mu->info();

  bool acc=true;

  //  cout<<fabs(info->getFloat("dxyBS"))<<"   "<<mu->d0(Primary)<<endl;

  if( fabs(mu->d0(Primary)) >= 0.2 )     acc=false;
  if(info->getFloat("normChi2") >= 10 ) acc=false;
  if(info->getInt("nMatch") <2 )    acc=false; 
  if(info->getBool("muidTrackerMuonArbitrated")!= true)  acc=false;
...
bool 
ZZAnalysis::MassSelection(Candidate* ZCand) {
...
Candidate *
ZZAnalysis::BestZCand( const CandList& zllList_o ) {
  
  CandList zllList = zllList_o;
  ...
  if(pdgId[0]==11 && pdgId[1]==11)
    Zmode = "Zee";
  else if(pdgId[0]==13 && pdgId[1]==13)
    Zmode = "Zmm";
  else 
...
CandList ZZAnalysis::MakeZCandidates() {
  ...
  for(int unsigned il1=0;il1<electrons.size();il1++) {
    ...
    for(int unsigned il2=0;il2<electrons.size();il2++) {
      if(il1 != il2) {
	Candidate* l2 = electrons[il2];
	...
	{
	  TVector3 v3_1m(0,0,0);
	  TVector3 v3_2m(0,0,0);

	  TVector3 v3_1p(0,0,0);
	  TVector3 v3_2p(0,0,0);
	  ...
	  float pt1m = KineUtils::et( l1->p()*(1-unc1), l1->eta() );
	  float pt1p = KineUtils::et( l1->p()*(1+unc1), l1->eta() );
	  float pt2m = KineUtils::et( l2->p()*(1-unc2), l2->eta() );
	  float pt2p = KineUtils::et( l2->p()*(1+unc2), l2->eta() );

	  v3_1m.SetPtEtaPhi( pt1m , l1->eta(), l1->phi() );
	  v3_1p.SetPtEtaPhi( pt1p , l1->eta(), l1->phi() );
	  v3_2m.SetPtEtaPhi( pt2m , l2->eta(), l2->phi() );
	  v3_2p.SetPtEtaPhi( pt2p , l2->eta(), l2->phi() );
	...
	Candidate* ZCandp = Candidate::create(l1p,l2p);
	Candidate* ZCandm = Candidate::create(l1m,l2m);
	
	ZCands.push_back(ZCand);
	ZCandsp.push_back(ZCandp);
	ZCandsm.push_back(ZCandm);
      }
    }
  }
 ...
 for(int unsigned il1=0;il1<muons.size();il1++) {
	...
	float unc = 0.04;
	{
	  ...
	  float pt1m = KineUtils::et( l1->p()*(1-unc), l1->eta() );
	  float pt1p = KineUtils::et( l1->p()*(1+unc), l1->eta() );
	  float pt2m = KineUtils::et( l2->p()*(1-unc), l2->eta() );
	  float pt2p = KineUtils::et( l2->p()*(1+unc), l2->eta() );
...
CandList ZZAnalysis::MakeZCandidatesWithCharge() {
...
TVector2
ZZAnalysis::ComputeRecoil(const Candidate* met, Candidate* ZCand) {
...
ZZAnalysis::initRootTree() {
 ...
 //Event
  ...
  outTree_.Branch("LeadAddElec",LeadAddElec,"LeadAddElec[5]/F");
  outTree_.Branch("LeadAddMuon",LeadAddMuon,"LeadAddMuon[5]/F");
  ...
  outTree_.Branch("tauTag",&tauTag,"tauTag/B");
...
void
ZZAnalysis::fillTree(Candidate* ZCand) {
...
bool
ZZAnalysis::ConversionRejection() {
...
void ZZAnalysis::fillLeptonTree(Candidate* ZCand)
{
  for(int ii=0;ii<2;ii++) 
	...
	/*	IsoVar[ii][0] = info->getFloat("isoR03strk");
	IsoVar[ii][1] = info->getFloat("isoR03strkVeto");
	IsoVar[ii][2] = info->getFloat("isoR03sum");*/
      }
  ...
  for(int unsigned iel=0;iel<electrons.size();iel++) {
	...
	{
	  LeadAddElec[0] = el->pt();
	  LeadAddElec[1] = el->eta();
	  LeadAddElec[2] = el->phi();

	  IsIsoElec = LepIso(el);
	  IsIDElec = LepID(el);
	  
	  Candidate* W = Candidate::create(el,met);
	  ElecMT = W->mass();
  ...
  for(int unsigned imu=0;imu<muons.size();imu++) {
	...
	{
	  LeadAddMuon[0] = mu->pt();
	  LeadAddMuon[1] = mu->eta();
	  LeadAddMuon[2] = mu->phi();

	  IsIsoMu = LepIso(mu);
	  IsIDMu = (LepID(mu)&MuonSpecID(mu));

	  Candidate* W = Candidate::create(mu,met);
	  MuMT = W->mass();
...
void ZZAnalysis::fillZTree(Candidate* ZCand)
{
  //Z Variables
  Z[0] = ZCand->pt();
  Z[1] = ZCand->eta();
...
void ZZAnalysis::fillMETTree(Candidate* ZCand)
{

  Candidate* met = (_e.met( EventManager::kPatMet , _vmet ))->clone();

  //take into account uncertainties on energy scale
  if(UNCERT!="") {
    TVector2 v2met = met->p2() - SavedCand->p2() + ZCand->p2();
    met->setP2( v2met );
  }
  /*  else { //or not !
    met = (_e.met( EventManager::kPatMet , _vmet );
    }*/
  //=================================================
  ...
  //MET related variables
  _e.refreshRecoMET();
  const Candidate* trkmet = _e.met( EventManager::kPatMet, 0 );
  TrkMET[0] = trkmet->pt();
  TrkMET[1] = trkmet->phi();
  _e.refreshRecoMET();
...
void ZZAnalysis::fillJetTree(Candidate* ZCand)
{
  ...
  for(int unsigned ij=0;ij<Jets.size();ij++) { 
    Candidate* jet = Jets[ij];
    {
      ...
      */
      if(jet->pt()*unc> 5 && dr1_>0.2 && dr2_>0.2 /*&& drE_>0.2 && drM_>0.2*/  )
	{

	  Jet.SetMagPhi(jet->pt()*unc, jet->phi() );
	  Ht += Jet;

	  if(jet->pt()*unc> 10) {
	    if(pttmp<jet->pt()*unc )
	      { theJet=jet; pttmp = jet->pt()*unc; }
	  }
	  if(jet->pt()*unc> 25) {
	    JetMultiplicity++;
	  }
  ...
  if(Jets.size()!=0 && pttmp!=0) {
    ...
    LJet[1] = theJet->eta();
    LJet[2] = theJet->phi();
    LJet[3] = 0;//info->getFloat("chargedHadronEnergyFraction") + info->getFloat("neutralHadronEnergyFraction");
...
void ZZAnalysis::fillPhotonTree(Candidate* ZCand)
{
...
void ZZAnalysis::fillEventTree(Candidate* ZCand)
{
...
void ZZAnalysis::ComputeCBMs(Candidate* ZCand) 
{
...
void ZZAnalysis::ZFrameVariables(Candidate* ZCand) {
...
void ZZAnalysis::fillMETPrime(Candidate* ZCand) {
...
void ZZAnalysis::fillProjectedMET(Candidate* ZCand) {
  ...
  CorProjMET =  (ProjMET - 0.8555*(Nvertex-1))/sqrt( 1 + (Nvertex-1)*0.178  );
  if( SampleName.substr(0,1)=="E" || SampleName.substr(0,1)=="M") { //data
    CorProjMET =  (ProjMET - 0.8555*(Nvertex-1))/sqrt( 1 + (Nvertex-1)*0.282  );
  }
...
void ZZAnalysis::FillGenEvent() {
  ...
  for(int unsigned imc=0;imc<mcCands.size();imc++) {
    ...
    if(abs(mcCand->pdgCode()) >=  11  && abs(mcCand->pdgCode()) <= 18  ) {
      ...
      if(abs(mcCand->pdgCode())==15 && mcCand->pt() > 20 && !tTag ) 
	{tauTag = true; tTag=true;}

      //  cout<<mcCand->pdgCode()<<"   "<<tauTag<<endl;
      if(abs(pdgid) == 23 || abs(pdgid) == 24 )
	{
	  ...
	  if( (abs(pdgid)==23 || pdgid==24) && (mcCand->pdgCode()==npdg[0] ||
						mcCand->pdgCode()==npdg[1] ||
						mcCand->pdgCode()==npdg[2] )  )
	    nZn = imc;
	  if((abs(pdgid)==23|| pdgid==-24) && (mcCand->pdgCode()==lpdg[0] ||
	  ...
	  //Rewriting for Ws
	  if(pdgid==24 )
	    if( mcCand->pdgCode()==npdg[0] || mcCand->pdgCode()==npdg[1] || mcCand->pdgCode()==npdg[2] )
	      W[0][0] = kl-1;
	    else
	      W[0][1] = kl-1;
	  else
  ...
  if(SampleName != "ZZ_2l2n")
    for(int ik=2;ik<2;ik++) {
      for(int k=0;k<5;k++) {
	McLepton[ik][k] =lepton[ W[ik][0] ][k];
	McNeutrino[ik][k] =lepton[ W[ik][1] ][k];
      }
    }
  // cout<<" debug 3 "<<endl;
...
Candidate* ZZAnalysis::FindBestdRCandidate(std::map<Candidate*, std::pair<float,float> > AssocMap ) {
...
int ZZAnalysis::OriginOfMCCandidate(Candidate* cand) {
...
Candidate* ZZAnalysis::MotherOfMCCandidate(Candidate* cand) {
...
float ZZAnalysis::ComputeResolution(float ElE) {
...
vector<float> ZZAnalysis::GetRValues(float ZPt, float Recoil[2], int Nv) {
...
vector<float> ZZAnalysis::GetRValuesFromData(float ZPt, float Recoil[2], int Nv) {
...
float ZZAnalysis::GetResponseForPFT1(float ZPt) {
  if(ZPt>3)
...
/*
void ZZAnalysis::GetElNNMETValue() {
...
*/
/*
void ZZAnalysis::InitializeElNNMET() {
